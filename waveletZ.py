...
plt.title("Filtragem de Ruído - Z Accel (g)")
plt.xlabel("Tempo (s)")
...
df_out = pd.DataFrame({
    "tempo_s": tempo,
    "z_original": sinal,
    "z_filtrado": sinal_filtrado
})

df_out.to_csv("saida_wavelet_Z.csv", index=False)
df_out.to_csv("saida_wavelet_Z.txt", sep="\t", index=False)
