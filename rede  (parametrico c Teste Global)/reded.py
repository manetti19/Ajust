...
# >>> Linha 4 com sentido invertido: em vez de +1.09 (A→D), usamos -1.09
linhas = [
    ['A', 'B', +6.16, 4],   # 1
    ['A', 'C', +12.57, 2],  # 2
    ['B', 'C', +6.41, 2],   # 3
    ['A', 'D', -1.09, 4],   # 4  (ERRO DE MODELO: sinal trocado)
    ['C', 'D', -11.58, 2],  # 5
    ['B', 'D', -5.07, 4]    # 6
]
...
variancias = 1e-3 * distancias      # σᵢ² = 10⁻³ * dᵢ
P = np.diag(1 / variancias)         # Pᵢ = 1000 / dᵢ
...
print("===== RESULTADOS DO AJUSTAMENTO (erro de modelo na linha 4) =====")
print(f"HB = {HB:.2f} m")
print(f"HC = {HC:.2f} m")
print(f"HD = {HD:.2f} m")
...
# Valores críticos informados (gl = 3, α = 5%)
chi2_min = 0.216
chi2_max = 9.35
