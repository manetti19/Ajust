...
plt.title("Filtragem de Ruído - Y Accel (g)")
plt.xlabel("Tempo (s)")
...
df_out = pd.DataFrame({
    "tempo_s": tempo,
    "y_original": sinal,
    "y_filtrado": sinal_filtrado
})

df_out.to_csv("saida_wavelet_Y.csv", index=False)
df_out.to_csv("saida_wavelet_Y.txt", sep="\t", index=False)
